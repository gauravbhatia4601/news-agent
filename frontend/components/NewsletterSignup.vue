      ...
      <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        ...
        <form @submit.prevent="handleSubmit" class="flex gap-2 w-full md:w-auto md:min-w-[360px]">
          <input
            ...
            class="flex-1 h-11 px-4 bg-background/10 border border-background/20 text-background placeholder-background/50 font-label text-sm focus-ring transition-colors"
            :disabled="status === 'loading' || status === 'success'"
          />
          <button
            type="submit"
            :disabled="status === 'loading' || status === 'success'"
            class="h-11 px-6 bg-secondary text-secondary-foreground font-label text-xs font-bold uppercase tracking-[0.062em] hover:opacity-90 transition-opacity disabled:opacity-50 whitespace-nowrap focus-ring"
          >
  ...
  <div v-else class="w-full">
    <form @submit.prevent="handleSubmit" class="flex gap-2">
      <input
        ...
        type="email"
        placeholder="you@example.com"
        class="flex-1 h-10 px-4 bg-muted border border-border text-foreground placeholder-muted-foreground font-label text-sm focus-ring transition-colors"
        :disabled="status === 'loading' || status === 'success'"
      />
      <button
        type="submit"
        :disabled="status === 'loading' || status === 'success'"
        class="h-10 px-5 bg-foreground text-background font-label text-xs font-bold uppercase tracking-[0.062em] hover:opacity-90 transition-opacity disabled:opacity-50 whitespace-nowrap focus-ring"
      >
        {{ status === 'loading' ? '...' : status === 'success' ? 'Done' : 'Subscribe' }}
      </button>
    </form>
