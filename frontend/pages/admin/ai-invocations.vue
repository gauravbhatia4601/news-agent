  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        <div class="w-10 h-10 rounded-xl bg-admin-sidebar/5 flex items-center justify-center"><Cpu class="w-5 h-5 text-admin-accent" /></div>
        <div>
          <h1 class="text-[18px] font-semibold text-admin-text">AI Invocations</h1>
          <p class="text-xs text-admin-muted">LLM call tracking, token usage, and cost monitoring</p>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <div class="bg-admin-surface rounded-[14px] border border-admin-border/80 shadow-[0_1px_2px_rgba(0,0,0,0.04)] p-5">
        <p class="text-2xl font-bold text-admin-text tracking-tight">{{ formatNumber(stats.total_tokens) }}</p>
        <p class="text-xs text-admin-muted mt-1">Total Tokens Used</p>
      ...
      <div class="bg-admin-surface rounded-[14px] border border-admin-border/80 shadow-[0_1px_2px_rgba(0,0,0,0.04)] p-5">
        <p class="text-2xl font-bold text-admin-text tracking-tight">{{ stats.total_invocations ?? 0 }}</p>
        <p class="text-xs text-admin-muted mt-1">Total Calls</p>
      ...
      <div class="bg-admin-surface rounded-[14px] border border-admin-border/80 shadow-[0_1px_2px_rgba(0,0,0,0.04)] p-5">
        <p class="text-2xl font-bold text-admin-text tracking-tight">{{ formatDuration(stats.avg_duration_ms) }}</p>
        <p class="text-xs text-admin-muted mt-1">Avg Duration</p>
      </div>
      <div class="bg-admin-surface rounded-[14px] border border-admin-border/80 shadow-[0_1px_2px_rgba(0,0,0,0.04)] p-5">
        <p class="text-2xl font-bold text-admin-text tracking-tight">{{ stats.by_provider?.length ?? 0 }}</p>
        <p class="text-xs text-admin-muted mt-1">Models Used</p>
      </div>
    </div>

    <!-- Provider Breakdown -->
    <div v-if="stats.by_provider?.length" class="bg-admin-surface rounded-[14px] border border-admin-border/80 shadow-[0_1px_2px_rgba(0,0,0,0.04)] p-6">
      <h2 class="text-[15px] font-semibold text-admin-text mb-4">Usage by Provider / Model</h2>
      <div class="space-y-3">
        <div v-for="p in stats.by_provider" :key="`${p.provider}-${p.model}`" class="flex items-center justify-between">
          <div>
            <span class="text-sm font-medium text-admin-text">{{ p.model }}</span>
            <span class="text-xs text-slate-400 ml-2">{{ p.provider }}</span>
          </div>
          <div class="flex items-center gap-4 text-xs">
            <span class="text-admin-muted">{{ p.count }} calls</span>
            <span class="font-medium text-admin-text">{{ formatNumber(p.tokens) }} tokens</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-admin-surface rounded-[14px] border border-admin-border/80 shadow-[0_1px_2px_rgba(0,0,0,0.04)] overflow-hidden">
      <div class="px-6 py-4 border-b border-admin-border flex items-center justify-between gap-4 flex-wrap">
        <h2 class="text-[15px] font-semibold text-admin-text">Invocation Log</h2>
      ...
      <div v-if="loading" class="p-12 text-center">
        ...
        <p class="text-sm text-admin-muted">Loading invocations...</p>
      </div>
      <div v-else-if="!invocations.length" class="p-12 text-center">
        <Cpu class="w-8 h-8 text-slate-300 mx-auto mb-3" />
        <p class="text-sm text-admin-muted">No AI invocations recorded yet</p>
      </div>
      <div v-else>
        <div class="divide-y divide-slate-100">
          <div v-for="inv in invocations" :key="inv.id" class="px-6 py-3.5 flex items-center gap-4 hover:bg-slate-50 transition-colors">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2">
                <span class="text-sm font-medium text-admin-text">{{ inv.model || 'Unknown' }}</span>
            ...
            <div class="text-right shrink-0">
              <p class="text-xs font-medium text-admin-text">{{ formatNumber(inv.total_tokens) }} tokens</p>
