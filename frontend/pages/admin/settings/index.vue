  ...
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-[14px] border border-slate-200/80 shadow-[0_1px_2px_rgba(0,0,0,0.04)] p-6">
      <div class="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 class="text-[15px] font-semibold text-slate-900">LLM Configuration</h2>
          <p class="text-xs text-slate-500">Provider and model used for article generation</p>
        </div>
      </div>
      ...
      <div v-else class="space-y-5">
        <!-- Provider -->
        <div>
          <label class="block text-[11px] font-semibold uppercase text-slate-500 tracking-wider mb-2">Primary Provider</label>
          <select
            v-model="form.provider"
            class="w-full px-4 py-2.5 text-sm bg-white border border-slate-200 rounded-[14px] focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-300 transition-all"
          >
            <option v-for="p in providers" :key="p.value" :value="p.value">
              {{ p.label }}{{ p.has_key ? '' : ' (no API key)' }}
            </option>
          </select>
          <p v-if="selectedProvider && !selectedProvider.has_key" class="mt-1.5 text-xs text-amber-600">
        ...
        <!-- Model -->
        <div>
          <label class="block text-[11px] font-semibold uppercase text-slate-500 tracking-wider mb-2">Primary Model</label>
          <input
            v-model="form.model"
            type="text"
        ...
        <!-- Timeout -->
        <div>
          <label class="block text-[11px] font-semibold uppercase text-slate-500 tracking-wider mb-2">Timeout (seconds)</label>
          <input
        ...
        <div class="flex items-center justify-between py-2">
          <div>
            <span class="text-sm font-medium text-slate-900">Generation Enabled</span>
            <p class="text-xs text-slate-500">Allow the engine to generate new articles</p>
          </div>
          <button
            @click="form.enabled = !form.enabled"
            class="relative w-11 h-6 rounded-full transition-colors duration-200"
            :class="form.enabled ? 'bg-slate-900' : 'bg-slate-200'"
          >
            <span
    ...
    <div class="bg-white rounded-[14px] border border-slate-200/80 shadow-[0_1px_2px_rgba(0,0,0,0.04)] p-6">
      <div class="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 class="text-[15px] font-semibold text-slate-900">Fallback Model</h2>
          <p class="text-xs text-slate-500">Used when the primary model fails (optional)</p>
        </div>
      </div>

      <div v-if="!loading" class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
          <label class="block text-[11px] font-semibold uppercase text-slate-500 tracking-wider mb-2">Fallback Provider</label>
          <select
        ...
        <div>
          <label class="block text-[11px] font-semibold uppercase text-slate-500 tracking-wider mb-2">Fallback Model</label>
          <input
    ...
    <div v-if="!loading" class="flex items-center justify-between">
      ...
      <button
        @click="saveSettings"
        :disabled="saving"
        class="flex items-center gap-2 px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-sm font-medium rounded-[14px] transition-colors disabled:opacity-50"
      >
    ...
    <div class="bg-white rounded-[14px] border border-slate-200/80 shadow-[0_1px_2px_rgba(0,0,0,0.04)] p-6">
      <div class="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 class="text-[15px] font-semibold text-slate-900">Quality Gate</h2>
          <p class="text-xs text-slate-500">Automated article quality thresholds</p>
        </div>
      </div>
      <div class="space-y-3">
        <div v-for="item in qualityItems" :key="item.label" class="flex items-center justify-between py-3 border-b border-slate-100 last:border-0">
