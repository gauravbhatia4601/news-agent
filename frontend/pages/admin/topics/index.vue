  ...
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-[14px] border border-slate-200/80 shadow-[0_1px_2px_rgba(0,0,0,0.04)] p-4 flex flex-wrap items-center gap-3">
      ...
      <input v-model="filters.search" type="search" placeholder="Search topics..."
        class="bg-slate-50 border border-slate-200 rounded-[14px] px-4 py-2.5 text-sm text-slate-900 focus:outline-none focus:border-slate-400 transition-colors flex-1 min-w-[200px]" />
      ...
      <span class="text-xs text-slate-500 font-medium">{{ topics.total || 0 }} topics</span>
    </div>
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="bg-slate-50/80 border-b border-slate-200">
            ...
            <th class="px-6 py-3.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Topic</th>
            ...
            <th class="px-6 py-3.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Sources</th>
            <th class="px-6 py-3.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Retries</th>
            <th class="px-6 py-3.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Article</th>
            <th class="px-6 py-3.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Updated</th>
        ...
        <tbody class="divide-y divide-slate-100">
          <tr v-for="t in topics.data" :key="t.id" class="hover:bg-slate-50 transition-colors" :class="{ 'bg-amber-50/40': selectedIds.includes(t.id) }">
            <td class="px-6 py-3.5"><input type="checkbox" :checked="selectedIds.includes(t.id)" @change="toggleOne(t.id)" class="rounded border-slate-400 text-slate-900 focus:ring-slate-400 bg-slate-50" /></td>
            <td class="px-6 py-3.5">
              <NuxtLink :to="`/admin/topics/${t.id}`" class="font-medium text-slate-900 hover:text-slate-700 transition-colors line-clamp-1">{{ t.topic_name }}</NuxtLink>
            </td>
            <td class="px-6 py-3.5 text-slate-500 whitespace-nowrap">{{ t.category || '—' }}</td>
            ...
            <td class="px-6 py-3.5 text-slate-500 text-center">{{ t.source_count }}</td>
            <td class="px-6 py-3.5 text-slate-500 text-center">{{ t.retry_count }}</td>
            <td class="px-6 py-3.5">
            ...
            <td class="px-6 py-3.5 text-xs text-slate-500 whitespace-nowrap">{{ formatDate(t.updated_at) }}</td>
            <td class="px-6 py-3.5 text-right">
              <div class="flex items-center justify-end gap-1">
                <button v-if="t.generation_status === 'pending'" @click="dispatchTopic(t.id)" class="p-1.5 text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Dispatch to queue"><Send class="w-3.5 h-3.5" /></button>
                <button v-if="t.generation_status === 'failed'" @click="retryTopic(t.id)" class="p-1.5 text-slate-500 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Retry"><RefreshCw class="w-3.5 h-3.5" /></button>
                <button @click="deleteTopic(t.id)" class="p-1.5 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete"><Trash2 class="w-3.5 h-3.5" /></button>
              </div>
            </td>
          </tr>
          <tr v-if="!topics.data?.length"><td colspan="9" class="px-6 py-12 text-center text-slate-500">
    ...
    <div v-if="topics.total > 0" class="flex items-center justify-between">
      ...
      <div class="flex gap-1">
        <button v-if="topics.current_page > 1" @click="filters.page = topics.current_page - 1" class="px-3 py-1.5 text-[11px] border border-slate-200 rounded-[14px] hover:bg-slate-50 text-slate-600 transition-colors uppercase tracking-wider">Previous</button>
        <template v-for="p in pageNumbers" :key="p">
          <button v-if="p === '...'" class="px-3 py-1.5 text-[11px] text-slate-400" disabled>...</button>
          <button v-else @click="filters.page = p" class="px-3 py-1.5 text-[11px] border rounded-[14px] transition-colors" :class="p === topics.current_page ? 'bg-slate-900 text-white border-slate-900' : 'border-slate-200 hover:bg-slate-50 text-slate-600'">{{ p }}</button>
        </template>
        <button v-if="topics.current_page < topics.last_page" @click="filters.page = topics.current_page + 1" class="px-3 py-1.5 text-[11px] border border-slate-200 rounded-[14px] hover:bg-slate-50 text-slate-600 transition-colors uppercase tracking-wider">Next</button>
      </div>
    </div>
  </div>
</template>
