  ...
  <div v-if="topic" class="space-y-6">
    ...
    <div class="bg-white rounded-lg border border-slate-200 p-6">
      <div class="flex items-start justify-between gap-4">
        <div>
          <h1 class="font-display text-2xl font-bold">{{ topic.topic_name }}</h1>
        ...
        <div class="flex gap-2">
          <button v-if="topic.generation_status === 'failed'" @click="retry"
            class="text-xs px-3 py-1.5 border border-amber-300 text-amber-700 rounded-md hover:bg-amber-50 transition-colors"
          >Retry</button>
          <button @click="deleteTopic"
            class="text-xs px-3 py-1.5 border border-red-300 text-red-700 rounded-md hover:bg-red-50 transition-colors"
          >Delete</button>
        </div>
      </div>
    </div>

    <div v-if="topic.article" class="bg-white rounded-lg border border-slate-200 p-6">
      <div class="flex items-center justify-between mb-3">
        <h2 class="font-display font-bold">Generated Article</h2>
      ...
      <dl class="grid grid-cols-2 gap-2 text-sm">
        <div><dt class="text-slate-500">Title</dt><dd>{{ topic.article.title }}</dd></div>
        <div><dt class="text-slate-500">Status</dt><dd><span class="text-xs px-2 py-0.5 rounded-full" :class="topic.article.status === 'published' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'">{{ topic.article.status }}</span></dd></div>
      </dl>
    </div>

    <div v-if="topic.sources?.length" class="bg-white rounded-lg border border-slate-200 p-6">
      <h2 class="font-display font-bold mb-4">Sources ({{ topic.sources.length }})</h2>
      <div class="space-y-3">
        <div v-for="s in topic.sources" :key="s.id" class="border border-slate-200 rounded-md p-3">
          <a :href="s.source_url" target="_blank" class="text-sm font-medium hover:text-blue-600 transition-colors">{{ s.headline || s.source_name }}</a>
